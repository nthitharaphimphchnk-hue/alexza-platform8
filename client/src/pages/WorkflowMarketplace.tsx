...
import { GitBranch, Search, Star, Download, User, Upload } from "lucide-react";
...
interface SectionsResponse {
  ok: boolean;
  sections: {
    trending: WorkflowMarketplaceItem[];
    popular: WorkflowMarketplaceItem[];
    new: WorkflowMarketplaceItem[];
  };
}
...
function WorkflowCard({
  ...
  onInstall: (item: WorkflowMarketplaceItem) => void;
  onRate?: (id: string, rating: number) => void;
  onPurchase?: (item: WorkflowMarketplaceItem) => void;
}) {
    ...
    <div className="group rounded-xl border border-[rgba(255,255,255,0.08)] bg-[#0b0e12]/70 p-5 hover:border-[rgba(192,192,192,0.3)] transition-all">
      <div className="flex items-start justify-between gap-2 mb-3">
        <div className="rounded-lg bg-[#c0c0c0]/10 p-2">
          <GitBranch size={20} className="text-[#c0c0c0]" />
        </div>
      ...
      <p className="text-sm text-gray-400 line-clamp-2 mb-2">{item.description || "No description."}</p>
...
export default function WorkflowMarketplace() {
  ...
  const [allItems, setAllItems] = useState<WorkflowMarketplaceItem[]>([]);
  const [categories, setCategories] = useState<string[]>([]);
  const [search, setSearch] = useState("");
  const [selectedCategory, setSelectedCategory] = useState("");
  const [isLoading, setIsLoading] = useState(true);
  const [searchMode, setSearchMode] = useState(false);
  const [selectedItem, setSelectedItem] = useState<WorkflowMarketplaceItem | null>(null);
  ...
  const [publishWorkflows, setPublishWorkflows] = useState<{ id: string; name: string }[]>([]);
  ...
  const loadSections = useCallback(async () => {
    setIsLoading(true);
    try {
      const data = await apiRequest<SectionsResponse>("/api/workflow-marketplace/sections?limit=6");
      ...
      showErrorToast("Failed to load workflow marketplace", error instanceof Error ? error.message : "Unknown error");
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const loadCategories = useCallback(async () => {
    try {
      const data = await apiRequest<{ ok: boolean; categories: string[] }>("/api/workflow-marketplace/categories");
  ...
  const loadSearch = useCallback(async () => {
    ...
    try {
      ...
      const data = await apiRequest<{ ok: boolean; items: WorkflowMarketplaceItem[] }>(
        `/api/workflow-marketplace?${params.toString()}`
      );
  ...
  const loadWorkflows = useCallback(async () => {
    try {
      const res = await apiRequest<{ ok: boolean; workflows: { id: string; name: string }[] }>("/api/workflows");
      setPublishWorkflows((res.workflows ?? []).map((w) => ({ id: w.id, name: w.name })));
      if (!publishWorkflowId && res.workflows?.[0]) setPublishWorkflowId(res.workflows[0].id);
    } catch {
      setPublishWorkflows([]);
    }
  }, [publishWorkflowId]);
  ...
  useEffect(() => {
    if (publishOpen) void loadWorkflows();
  }, [publishOpen, loadWorkflows]);

  const handleInstall = async () => {
    if (!selectedItem || !applyWorkspaceId) return;
    setIsInstalling(true);
    try {
      const res = await apiRequest<{ ok: boolean; workflow: { id: string; name: string; workspaceId: string } }>(
        `/api/workflow-marketplace/${selectedItem.id}/install`,
        { method: "POST", body: { workspaceId: applyWorkspaceId } }
      );
      showSuccessToast("Installed", `Workflow "${res.workflow?.name}" added to workspace`);
      setInstallDialogOpen(false);
      setSelectedItem(null);
      setApplyWorkspaceId(workspaceOptions[0]?.id ?? "");
      setLocation("/app/workflows");
      void loadSections();
    } catch (error) {
      if (error instanceof ApiError && error.status === 401) {
        window.location.href = "/login";
  ...
  const handlePurchase = async (item: WorkflowMarketplaceItem) => {
    try {
      const res = await apiRequest<{ ok: boolean; url: string | null }>(`/api/workflow-marketplace/${item.id}/purchase`, { method: "POST" });
      if (res.url) {
  ...
  const handleRate = async (id: string, rating: number) => {
    try {
      await apiRequest<{ ok: boolean; rating: number; ratingCount: number }>(
        `/api/workflow-marketplace/${id}/rate`,
  ...
  const handlePublish = async () => {
    if (!publishWorkflowId) return;
    setIsPublishing(true);
    try {
      const tags = publishTags.split(",").map((t) => t.trim()).filter(Boolean);
      await apiRequest<{ ok: boolean }>("/api/workflow-marketplace/publish", {
        method: "POST",
        body: {
      ...
      showSuccessToast("Published", "Workflow published to marketplace");
      setPublishOpen(false);
      setPublishWorkflowId("");
      setPublishName("");
      setPublishDesc("");
      setPublishCategory("");
      setPublishTags("");
  ...
  const openInstallDialog = (item: WorkflowMarketplaceItem) => {
  ...
  return (
    <AppShell
      ...
      subtitle="Browse and install automation workflows"
    ...
    >
        ...
        {isLoading && !searchMode ? (
          ...
          <>
            {sections.trending.length > 0 && (
              <section>
                <h2 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
                  <span className="text-[#c0c0c0]">↑</span> Trending Workflows
                </h2>
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            {sections.popular.length > 0 && (
              <section>
                <h2 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
                  <Download size={18} className="text-[#c0c0c0]" /> Popular Workflows
                </h2>
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            {sections.new.length > 0 && (
              <section>
                <h2 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
                  <span className="text-[#c0c0c0]">◆</span> New Workflows
                </h2>
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  ...
                  {categories.map((c) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        setAllItems([]);
                        setIsLoading(true);
                        apiRequest<{ ok: boolean; items: WorkflowMarketplaceItem[] }>(
                          `/api/workflow-marketplace?category=${encodeURIComponent(c)}&limit=50`
                        )
            ...
            {sections.trending.length === 0 && sections.popular.length === 0 && sections.new.length === 0 && (
              <div className="rounded-xl border border-[rgba(255,255,255,0.08)] bg-[#0b0e12]/70 p-12 text-center">
                <GitBranch size={48} className="mx-auto text-gray-500 mb-4" />
                <p className="text-gray-400">No workflows in the marketplace yet.</p>
                <p className="mt-1 text-sm text-gray-500">Publish your first workflow to get started.</p>
              </div>
            )}
          </>
        ) : null}
      ...
      <Dialog open={installDialogOpen} onOpenChange={setInstallDialogOpen}>
        <DialogContent className="bg-[#0b0e12] border-[rgba(255,255,255,0.08)] max-w-[480px]">
          <DialogHeader>
            <DialogTitle className="text-white">Install Workflow</DialogTitle>
          </DialogHeader>
          {selectedItem && (
            <div className="space-y-4">
              <div>
                <p className="text-sm text-gray-400 mb-1">Workflow</p>
        ...
        <DialogContent className="bg-[#0b0e12] border-[rgba(255,255,255,0.08)] max-w-[480px]">
          ...
          <div className="space-y-4">
            <div>
              ...
              <Select value={publishWorkflowId} onValueChange={setPublishWorkflowId}>
                <SelectTrigger className="mt-2 w-full border-[rgba(255,255,255,0.12)] bg-[#050607] text-white">
                  <SelectValue placeholder="Select workflow" />
                </SelectTrigger>
                <SelectContent>
                  {publishWorkflows.map((w) => (
                    <SelectItem key={w.id} value={w.id}>
                      {w.name}
                    </SelectItem>
                  ))}
            ...
            <div>
              ...
              <input
                type="text"
                placeholder="automation, webhook, schedule"
                value={publishTags}
            ...
            <div className="flex gap-2 pt-2">
              ...
              <Button
                ...
                disabled={!publishWorkflowId || isPublishing}
                className="flex-1 bg-[#c0c0c0] text-black hover:bg-[#a8a8a8]"
              >
